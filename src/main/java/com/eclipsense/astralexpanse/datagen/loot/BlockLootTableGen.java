...
public class BlockLootTableGen extends BlockLootSubProvider {
    ...
    protected void generate() {
        dropSelf(AstralBlocks.VOIDINIUM_BLOCK.get());
        add(AstralBlocks.END_STONE_VOIDINIUM_ORE.get(), createMultipleOreDropsWithRareDrop(
                AstralBlocks.END_STONE_VOIDINIUM_ORE.get(),
                AstralItems.VOIDINIUM_SHARD.get(),
                AstralItems.CONCENTRATED_VOIDINIUM.get(),
                1.0f,
                3.0f,
                0.10f));
    ...
    protected Iterable<Block> getKnownBlocks() {
        return AstralBlocks.BLOCKS.getEntries().stream().map(Holder::value)::iterator;
    }
}
